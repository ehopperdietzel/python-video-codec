...
# Genera matriz de cuantización dado un porcentaje de calidad
def genQ(percent):
...
# Returna último valor distinto de cero
...
# Erro cuadrático medio con normalización previa
def MSENorm(a,b):
    a = a.astype('float32')
    a = a/np.amax(a)
    b = b.astype('float32')
...
def huffman(data,dendograma):
    ...
    # Fenera el arreglo char de bits
    for n in data:
        coded += dendograma[n]

    # Verifica que el largo sea múltiplo de 8
    missingBits = 8 - len(coded)%8

    # Completa con 0 en caso contrario
